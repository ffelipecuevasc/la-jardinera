# Iteración 08: Header Móvil Esencial y Panel Anclado en "servicios.html"
...
variantes. En `servicios.html` persiste el defecto original: el drawer `fixed inset-0` está anidado en el `<header>`
con `backdrop-blur-xl`, así que se mide contra la barra de 80 px y sus enlaces se desbordan. Tu trabajo es replicar aquí
los cambios de `index.html`, ajustando únicamente el estado activo a "Servicios". `navigation.js` ya es compatible:
**no lo modifiques**. Interviene quirúrgicamente **únicamente `servicios.html`**; `<main>`, `<footer>` y `modal.js`
permanecen intactos.

## 1. Objetivo de la Iteración

Replicar en `servicios.html` el header maestro de la Fase 2 (cabecera esencial en móvil y panel anclado bajo la
barra con capa tenue), dejando "Servicios" como único enlace activo en la barra de escritorio y en el panel móvil, sin
alterar el catálogo ni el modal dinámico.

## 2. Tareas Técnicas (Ejecución Estricta)

1. **Compactación del Bloque de Marca en `servicios.html`:**
...
3. **Sustitución del Drawer por el Panel Anclado y la Capa Tenue:**
    ...
    * Inyectar, inmediatamente antes de `</header>`, el panel estandarizado con "Servicios" como ítem activo:
      ```html
...
4. **Auditoría de No-Regresión e Invariantes:**
    * **Contenido de la página:** la grilla de las 8 tarjetas del catálogo con sus atributos `data-service-id`, la
      franja de beneficios y el modal accesible `#service-modal` (galería dinámica y Focus Trap) permanecen 100%
      intactos.
    * **Convivencia de `Escape`:** con el modal abierto, `Escape` cierra solo el modal; con el menú abierto, `Escape`
      cierra solo el menú. Nunca coexisten abiertos (la capa tenue impide abrir el modal con el menú desplegado).
    * **Escritorio intacto:** la `<nav class="hidden lg:flex ...">` (con "Servicios" activo y su subrayado `scale-x-100`) y el `<footer>` permanecen sin cambios; a partir de `lg` el header luce idéntico al anterior.
...
* **Escenario 1: Cabecera Esencial sin Desbordes**
    * **Dado** el archivo `servicios.html` en viewports de 320, 360 y 412 px de ancho (y de 768 px)
...
* **Escenario 2: Panel Anclado, Legible y con Estado Activo**
    * **Dado** el archivo `servicios.html` en un viewport móvil, sobre el catálogo de servicios, en modo claro y en modo oscuro
    ...
    * **Entonces** el panel aparece inmediatamente bajo la barra, a ancho completo y con fondo sólido; se ven los 5
      enlaces (con "Servicios" en `text-primary font-semibold` y `aria-current="page"`) y los botones WhatsApp e Instagram, todos con texto legible (contraste WCAG AA); el
...
* **Escenario 3: Comportamiento Consistente del Módulo Compartido**
    * **Dado** el menú abierto en `servicios.html`
...
* **Escenario 4: Cero Regresión en Escritorio y en el Catálogo**
    * **Dado** `servicios.html` a `>= 1024 px` y en móvil con el menú cerrado, en modo claro y oscuro
    * **Cuando** el usuario recorre la página, alterna el tema y pulsa "Ver más" en una tarjeta de servicio
    * **Entonces** el header de escritorio luce idéntico al anterior (con "Servicios" activo); el modal `#service-modal` se abre, renderiza su galería, atrapa el foco y se cierra sin interferencias del menú móvil; y la consola no registra errores.