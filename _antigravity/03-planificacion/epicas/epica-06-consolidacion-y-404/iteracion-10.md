# Iteración 10: Header Móvil Esencial y Panel Anclado en "galeria.html"
...
variantes. En `galeria.html` persiste el defecto original: el drawer `fixed inset-0` está anidado en el `<header>` con
`backdrop-blur-xl`, así que se mide contra la barra de 80 px y sus enlaces se desbordan. Tu trabajo es replicar aquí los
cambios de `index.html`, ajustando únicamente el estado activo a "Galería". `navigation.js` ya es compatible: **no lo
modifiques**. Interviene quirúrgicamente **únicamente `galeria.html`**; `<main>`, `<footer>` y `gallery.js` permanecen
intactos.

## 1. Objetivo de la Iteración

Replicar en `galeria.html` el header maestro de la Fase 2 (cabecera esencial en móvil y panel anclado bajo la
barra con capa tenue), dejando "Galería" como único enlace activo en la barra de escritorio y en el panel móvil, sin
alterar el acordeón de tarjetas ni el Lightbox multimodal.

## 2. Tareas Técnicas (Ejecución Estricta)

1. **Compactación del Bloque de Marca en `galeria.html`:**
...
3. **Sustitución del Drawer por el Panel Anclado y la Capa Tenue:**
    ...
    * Inyectar, inmediatamente antes de `</header>`, el panel estandarizado con "Galería" como ítem activo:
      ```html
...
4. **Auditoría de No-Regresión e Invariantes:**
    * **Contenido de la página:** las 34 tarjetas interactivas conservan su efecto acordeón (`flex-1 hover:grow-[10]`),
      sus atributos `data-type` y `data-src`, y el visor `#lightbox` sigue abriendo imágenes y videos sin bloqueos.
    * **Convivencia de `Escape`:** con el Lightbox abierto, `Escape` cierra solo el Lightbox; con el menú abierto,
      `Escape` cierra solo el menú. Nunca coexisten abiertos.
    * **Escritorio intacto:** la `<nav class="hidden lg:flex ...">` (con "Galería" activo y su subrayado `scale-x-100`) y el `<footer>` permanecen sin cambios; a partir de `lg` el header luce idéntico al anterior.
...
* **Escenario 1: Cabecera Esencial sin Desbordes**
    * **Dado** el archivo `galeria.html` en viewports de 320, 360 y 412 px de ancho (y de 768 px)
...
* **Escenario 2: Panel Anclado, Legible y con Estado Activo**
    * **Dado** el archivo `galeria.html` en un viewport móvil, sobre el encabezado editorial y la galería, en modo claro y en modo oscuro
    ...
    * **Entonces** el panel aparece inmediatamente bajo la barra, a ancho completo y con fondo sólido; se ven los 5
      enlaces (con "Galería" en `text-primary font-semibold` y `aria-current="page"`) y los botones WhatsApp e Instagram, todos con texto legible (contraste WCAG AA); el
...
* **Escenario 3: Comportamiento Consistente del Módulo Compartido**
    * **Dado** el menú abierto en `galeria.html`
...
* **Escenario 4: Cero Regresión en Escritorio y en la Galería**
    * **Dado** `galeria.html` a `>= 1024 px` y en móvil con el menú cerrado, en modo claro y oscuro
    * **Cuando** el usuario recorre las 6 categorías y abre una imagen y un video en el Lightbox
    * **Entonces** el header de escritorio luce idéntico al anterior (con "Galería" activo); el acordeón y el Lightbox se comportan como antes, con cierre correcto y sin fugas de nodos; y la consola no registra errores.