# Iteración 09: Header Móvil Esencial y Panel Anclado en "suscripcion-floral.html"
...
variantes. En `suscripcion-floral.html` persiste el defecto original: el drawer `fixed inset-0` está anidado en el
`<header>` con `backdrop-blur-xl`, así que se mide contra la barra de 80 px y sus enlaces se desbordan. Tu trabajo es
replicar aquí los cambios de `index.html`, ajustando únicamente el estado activo a "Suscripción Floral". `navigation.js`
ya es compatible: **no lo modifiques**. Interviene quirúrgicamente **únicamente `suscripcion-floral.html`**; `<main>`,
`<footer>` y `subscription.js` permanecen intactos.

## 1. Objetivo de la Iteración

Replicar en `suscripcion-floral.html` el header maestro de la Fase 2 (cabecera esencial en móvil y panel anclado
bajo la barra con capa tenue), dejando "Suscripción Floral" como único enlace activo en la barra de escritorio y en el
panel móvil, sin alterar el Hero dinámico ni el acordeón de planes.

## 2. Tareas Técnicas (Ejecución Estricta)

1. **Compactación del Bloque de Marca en `suscripcion-floral.html`:**
...
3. **Sustitución del Drawer por el Panel Anclado y la Capa Tenue:**
    ...
    * Inyectar, inmediatamente antes de `</header>`, el panel estandarizado con "Suscripción Floral" como ítem activo:
      ```html
...
4. **Auditoría de No-Regresión e Invariantes:**
    * **Contenido de la página:** el carrusel con cross-fade del Hero (`#banner-carousel`), la sección de Condiciones y
      Zonas de Envío, el botón `#show-plans-btn`, la sección colapsable `#planes` y el botón `#hide-plans-btn` siguen
      funcionando bajo el control de `subscription.js`, incluido el desplazamiento suave (`scrollIntoView`) con la
      barra fija.
    * **Escritorio intacto:** la `<nav class="hidden lg:flex ...">` (con "Suscripción Floral" activo y su subrayado `scale-x-100`) y el `<footer>` permanecen sin cambios; a partir de `lg` el header luce idéntico al anterior.
...
* **Escenario 1: Cabecera Esencial sin Desbordes**
    * **Dado** el archivo `suscripcion-floral.html` en viewports de 320, 360 y 412 px de ancho (y de 768 px)
...
* **Escenario 2: Panel Anclado, Legible y con Estado Activo**
    * **Dado** el archivo `suscripcion-floral.html` en un viewport móvil, sobre el Hero con carrusel, en modo claro y en modo oscuro
    ...
    * **Entonces** el panel aparece inmediatamente bajo la barra, a ancho completo y con fondo sólido; se ven los 5
      enlaces (con "Suscripción Floral" en `text-primary font-semibold` y `aria-current="page"`) y los botones WhatsApp e Instagram, todos con texto legible (contraste WCAG AA); el
...
* **Escenario 3: Comportamiento Consistente del Módulo Compartido**
    * **Dado** el menú abierto en `suscripcion-floral.html`
...
* **Escenario 4: Cero Regresión en Escritorio y en el Hero/Planes**
    * **Dado** `suscripcion-floral.html` a `>= 1024 px` y en móvil con el menú cerrado, en modo claro y oscuro
    * **Cuando** el usuario observa el fundido del Hero y pulsa "Ver Planes de Suscripción" y luego el botón de colapsar
    * **Entonces** el header de escritorio luce idéntico al anterior (con "Suscripción Floral" activo); el cross-fade, la expansión y el colapso de `#planes` funcionan sin colisiones con `navigation.js`; y la consola no registra errores.